...
const NewsSection = (request) => {
    ...
    return (
        <div>
            <Row>
                <Col>
                    <h1 style={{ fontSize: '30px' }} className="headingSec">{request.topHeading}</h1>
                </Col>
            </Row>
            <Row>
                    ...
                    newsSection.map((article, key) =>
                        article.urlToImage === "" || article.urlToImage === null ? null :
                            <Col key={key} md={{ span: 8 }} sm={{ span: 24 }} >
                                <Card className="cardNews" onClick={() => window.open(article.url, "_blank")} type="inner" hoverable="true"
